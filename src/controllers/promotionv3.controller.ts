...
import { PromotionV3 as Promotion, PromotionStatus, PromotionType } from '../models/promotion-v3.model';
import { PromotionRuleV3 as PromotionRule } from '../models/promotion-rule-v3.model';
...
export class PromotionController {
    ...
    try {
      ...
      if (sourceRules.length > 0) {
        const newRules = sourceRules.map(sourceRule => {
          ...
          newRule.contents = sourceRule.contents;
          newRule.promotionId = savedPromotion.id;
          newRule.description = sourceRule.description;
          newRule.priority = sourceRule.priority;
          newRule.exclusive = sourceRule.exclusive;
          return newRule;
        });
  ...
  async addRule(req: Request, res: Response): Promise<Response> {
    try {
      const { name, promotionId, contents, description, priority, exclusive } = req.body;


      // 验证必要字段
      if (!name || !promotionId || contents === undefined) {
        return errorResponse(res, 400, '缺少必要参数', null);
      }

      // contents 不能是 ""
      if (contents === '""') {
        return errorResponse(res, 400, 'contents 不能为空字符串', null);
      }
      ...
      try {
        ...
        return errorResponse(res, 400, 'contents 不是有效的 JSON 字符串', null);
      }
      ...
      rule.exclusive = exclusive === "true" ? 1 : 0;
      rule.contents = "{}";

      const ruleRepository = AppDataSource.getRepository(PromotionRule);
      // 保存规则
      const savedRule = await ruleRepository.save(rule);

      contentsObj.id = Number(savedRule.id)
  ...
  async updateRule(req: Request, res: Response): Promise<Response> {
    try {
      const { id } = req.params;
      const { name, type, contents, description, priority, exclusive } = req.body;
      ...
      if (contents !== undefined) rule.contents = contents;
  ...
  async convertRule(req: Request, res: Response): Promise<Response> {
    ...
    try {
      ...
      const promotionRules = discountRules.map(rule => {
        ...
        promotionRule.contents = JSON.stringify(rule.contents);
